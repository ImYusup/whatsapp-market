...
import {
  twelveDataProvider,
} from "@/lib/market/providers/twelveData";
...
async function sendToAppsScript(
  ...
  if (!url) {
    throw new Error(
      "APPS_SCRIPT_WEBHOOK_URL belum diset."
    );
  }

  if (!secret) {
    throw new Error(
      "APPS_SCRIPT_WEBHOOK_SECRET belum diset."
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Apps Script HTTP ${response.status}: ${body}`
    );
